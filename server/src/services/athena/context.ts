...
export async function buildSystemPrompt(
  ...
  const sandboxGuideline = isPaidTier
    ...
    : "";
  // Paid-tier app capabilities (Grades, Habits, Pomodoro, Ntfy, Reminders,
  // Browser, Maps). Hidden from FREE/DEMO users so the model never advertises
  // tools it can't invoke (the tools are filtered out by toolsForRole).
  const gradesLine = isPaidTier
    ? "- Grades: list_courses, get_course_grades\n"
    : "";
  const habitsLine = isPaidTier
    ? "- Habits: list_habits, create_habit, log_habit, delete_habit\n"
    : "";
  const focusLine = isPaidTier
    ? "- Focus: start_pomodoro (opens the Pomodoro timer on the user's desktop)\n"
    : "";
  const ntfyLine = isPaidTier
    ...
    : "";
  const remindersLine = isPaidTier
  ...
  • Workflow for "find water/sleeping spots near X": geocode X → find_nearby_pois(categories="water" or "sleeping") → show_map_pois → list them in your reply.\n`
    : "";
  const scheduleReviewLine = isPaidTier
    ? " schedule_note_review (schedule a calendar event to review a note)."
    : "";
  // Paid-tier guideline lines (hidden from FREE/DEMO users).
  const pomodoroGuideline = isPaidTier
    ? "- For start_pomodoro, just call the tool — the timer opens automatically on the user's desktop.\n"
    : "";
  const remindersGuideline = isPaidTier
    ...
    : "";
  const browserGuideline = isPaidTier
  ...
  const pulseLine = isProTier
    ...
    : "";
  const forgeLine = isProTier
    ? "- Forge (Pro — AI practice problem generator): forge_list_sets (list the user's practice problem sets), forge_get_set (get a set's problems with solutions), forge_generate (generate new problems from a source — note, file, Atlas concepts, or free text; supports mcq/short_answer/step_by_step/mixed formats and adaptive difficulty that targets weak Atlas concepts), forge_stats (practice stats: sets, problems, attempts, avg score), forge_attempts (recent attempt history with feedback), open_forge (open the Forge app, optionally focused on a set). Use forge_generate when the user asks to 'quiz me', 'generate practice problems', 'test my knowledge on X', or wants to practice before an exam. Forge integrates with Atlas (targets weak concepts) and Pulse (mastery signals).\n"
    : "";
  const bridgeLine = isProTier
    ? "- Concept Bridge (Pro — interdisciplinary connection surfacer): bridge_list (list cross-course concept connections the AI discovered), bridge_discover (run AI discovery to find new interdisciplinary connections in the user's Atlas — use when the user asks 'what connects my courses?' or 'find connections between my subjects'), bridge_stats (bridge counts by relation type + unseen count), bridge_for_concept (find bridges involving a specific concept), bridge_mark_all_seen (dismiss bridge notifications), open_bridge (open the Concept Bridge app). Bridge finds non-obvious links between concepts from different courses (e.g. 'eigenvalues from Linear Algebra explain why PCA works in Machine Learning'). Proactively run bridge_discover when the user has a rich Atlas (4+ concepts from 2+ sources) and hasn't discovered bridges yet.\n"
    : "";
  const scribeLine = isProTier
    ? "- Scribe (Pro — thesis/essay writing coach): scribe_list_documents (list the user's writing drafts), scribe_get_document (get a document's full content + feedback passes), scribe_create_document (create a new essay/thesis/report draft — optionally linked to a Compass project for citation gap detection), scribe_update_document (update draft content/metadata), scribe_generate_feedback (kick off AI writing feedback — types: outline, draft, citations, full; fire-and-forget, poll with scribe_get_feedback), scribe_get_feedback (poll feedback status + get full Markdown feedback + issues + score when ready), open_scribe (open the Scribe app, optionally focused on a document). Use scribe_generate_feedback when the user asks 'review my essay', 'what's wrong with my draft?', 'check my citations', or wants writing help. Scribe integrates with Compass (citation gaps) and Atlas (concept coverage).\n"
    : "";
  const circleLine = isProTier
...
${browserBlock}${sandboxLine}- Auto notetaking: create_notes_from_url (fetch a web page → AI generates structured notes → saves + opens Notes), create_notes_from_pdf (extract text from an uploaded PDF → AI notes → saves + opens Notes). Styles: cornell / outline / summary / bullets.
- Cross-app composites: create_task_from_note (extract one task from a note), create_tasks_from_note (extract multiple tasks), create_note_from_task (expand a task into a note),${scheduleReviewLine}
...
${mapsBlock}${atlasLine}${crunchLine}${compassLine}${echoLine}${pulseLine}${forgeLine}${bridgeLine}${scribeLine}${circleLine}
Guidelines:
- SAFETY (self-harm & crisis): If the user expresses thoughts of self-harm, suicide, or being in crisis, respond with empathy and care. Do NOT attempt to diagnose or provide therapy. Do NOT dismiss or minimize their feelings. Share crisis resources clearly: Czech crisis line 116 123 (free, 24/7, Czech + English), emergency 112, and https://www.linka-bezpeci.cz. If they are in immediate danger, urge them to call 112 or go to the nearest emergency room. Take every such statement seriously — never joke about it or change the subject.
...
${remindersGuideline}${browserGuideline}- Use Markdown for formatting responses.
