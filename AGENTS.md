...
- `OPENAI_PROVIDER` / `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL` — server-wide LLM fallback. In `hybrid` LLM mode (admin-configured) these power the **hosted AI** tier that Mavino funds for users; in `per-user`/`global` modes they are the fallback for users without keys. `APP_BASE_URL` is required for guardian-consent links and the OpenRouter OAuth callback; `SMTP_URL`/`SMTP_FROM` for consent e-mails; `OPENAI_TRANSCRIPTION_*` for Whisper.
...
│       ├── routes/{auth, notes, tasks, files, spotify, lyrics, flashcards, grades, ai, athena, conversations, study, calendar, habits, capture, microsoft, whiteboards, ntfy, voice, browser, focus, analytics}.ts
│       ├── services/{spotify.ts, lrclib.ts, jwt.ts, crypto.ts, microsoft.ts, browser.ts, ntfy/{client, config, scheduler, subscriber, athena-turn}.ts}
...
└── client/
    └── src/
        ...
        ├── services/             # API clients (api, notes, tasks, files, spotify, lyrics, flashcards, grades, athena, conversations, study, calendar, habits, microsoft, users, whiteboards, ntfy, voice, browser, focus, analytics)
...
10. **Settings** — Split into 11 sections (sidebar nav, `apps/settings/sections/*.tsx`):
    ...
    - **Integrations** — consolidated connect/disconnect/status for Spotify (server-wide), Microsoft Calendar (server-wide + sync). Reuses existing per-app client services.
...
11. **Study Hub** — AI study workflows (one-click, structured) on top of the Athena LLM infra (`services/athena/llm.ts`):
    ...
    - **Ask (grounded)** — NotebookLM-style source-grounded Q&A. Pick sources (notes/files/PDFs/URLs/pasted text) → ask questions → streamed answers cite the source for every claim with clickable `[n]` chips. Conversations are persisted as `StudyChat` rows (resumable). Backend: `routes/study-chat.ts` (`/api/study/chat/*`) with SSE streaming (`POST /:id/stream`); sources injected as numbered SOURCE blocks via `groundedQaSystemPrompt`; citations extracted + persisted per assistant message. Client: `apps/study/SourceChat.tsx` + `CitationMarkdown.tsx` (renders `[n]` as superscript chips that open the source).
    - **Podcast** — generates a configurable 2-host dialogue script from selected sources (LLM), saves the script as a Note, then renders a persistent downloadable MP3 with AWS Polly. Users choose language, length, tone, focus, host names, Polly engine, and compatible voices discovered from AWS. Audio rendering runs as a background job, synthesizes individual turns, and concatenates them with ffmpeg; the client polls status and plays authenticated audio blobs. Global AWS credentials are encrypted in the `Setting` table and configured by admins in Settings → Study Hub, with `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION` as environment fallbacks. Backend: `routes/study-podcasts.ts`, `routes/admin-polly.ts`, `services/study/podcast-audio.ts`, and `services/polly-config.ts`. Client: `apps/study/Podcast.tsx`.
    - **Source library** — persistent `StudySource` rows (note/file/paste/url with cached extracted text) reusable across grounded Q&A, podcasts, and cited study materials. Deduped by (userId, kind, refId); re-resolving refreshes the cached text. Backend: `routes/study-sources.ts` (`/api/study/sources/*`) + `resolveAndCache` in `services/study/source.ts`. Source resolution now supports **PDF extraction** (via `pdf-parse` v2) and **URL fetching** (via `@postlight/parser` Readability, reusing `services/fetcher.ts`); `MAX_SOURCE_CHARS` bumped to 30000.
    ...
    - Backend: `routes/study.ts` (`/api/study/*`) reuses `getUserConfig`/`buildModel`; structured JSON endpoints via `services/study/llm-json.ts` (robust JSON extraction + one re-prompt retry); source resolution in `services/study/source.ts` (note/file/PDF/paste/URL); in-memory quiz store (`services/study/quiz-store.ts`, 30-min TTL). Citation-aware prompt variants in `services/study/prompts.ts` (`*CitedPrompt` functions) keep generated materials faithful to sources.
    ...
    - Athena tools: `generate_flashcards` (creates deck + opens Flashcards via client_action), `summarize_note`, `create_tasks_from_text`, `open_study_hub` (opens Study Hub with optional preselected mode/source).
...
12. **Athena** — LLM workspace assistant (chat UI) powered by `multi-llm-ts`:
    ...
    - Any `multi-llm-ts` provider works (openai, deepseek, anthropic, openrouter, ollama, groq, mistralai, google, xai, meta, cerebras). Per-user config encrypted (AES-256-GCM) in DB. In **hybrid LLM mode** (`services/llm-config.ts`), users pick Mavino-hosted AI (OpenAI-backed, metered against per-tier monthly budgets in `services/llm-budget.ts`, admin kill switch) or BYOK (adults can connect OpenRouter via OAuth PKCE in `services/openrouter-oauth.ts`). Minors 13–17 require guardian e-mail consent (`services/guardian-consent.ts`) and are forced to hosted + OpenAI moderation (`services/llm-safety.ts`). All `acquireLlmModel`/`acquireMeteredProvider` calls in `services/athena/llm.ts` are metered to `LlmUsage` — never call `buildModel`/provider endpoints directly.
...
13. **Calendar / Planner** — Unified time-based view for all student events:
    ...
    - **Layer toggles** to show/hide: My Events (manual), Tasks (due dates), Assignments, Microsoft (Outlook sync).
    ...
    - Athena tools: `list_calendar_events`, `create_calendar_event`, `schedule_task` (links a task to a time slot + opens Calendar via client_action), `find_free_slots` (reads events for a day, returns free windows), `open_calendar`, `sync_microsoft_calendar` (pulls MS events into local DB, returns synced/deleted counts).
    - Today app integration: "Today's Schedule" card shows today's events sorted by start time.
14. **Habit Tracker** — Streak-based daily/weekly habits:
...
15. **Quick Capture** — Global capture inbox:
...
16. **Whiteboard** — Interactive vector drawing canvas for learning/sketching:
...
17. **Ntfy** — Bidirectional push-notification channel for Athena + scheduled cron jobs:
...
18. **Voice Notes** — Microphone recorder → transcribed linked Note:
...
19. **Browser** — Athena-integrated web browser (`apps/browser/BrowserApp.tsx`):
    - Desktop web browser rendered through a backend reverse proxy. Pages are fetched server-side, rewritten so all navigation stays inside the proxy, and served to a sandboxed `<iframe>` — bypassing `X-Frame-Options`/CSP so most sites embed cleanly.
...
20. **Analytics & Gamification** — Unified dashboard (`apps/analytics/AnalyticsApp.tsx`) aggregating the user's own data across Habits, Pomodoro, Flashcards, Grades, Study Hub, and Tasks into charts, plus an XP/level system + achievements.
...
Apps are classified into subscription tiers (defined in `client/src/apps/registry.tsx` `minTier` fields, mirrored server-side in `server/src/services/features.ts`):
...
**Flag storage:** all flags live in the `Setting` key/value table. Per-user flags use the user's id; global flags use `userId = null`. Keys: `app.tiers` (global, JSON appId→tier overrides), `apps.disabled` (global, JSON string array), `stripe.price.paid` / `stripe.price.pro` (global, admin-configured Stripe price IDs).
...
**Server routes** (`server/src/routes/features.ts`, mounted at `/api/features`): `GET /` (own state: subscriptionTier, disabledApps, appTiers), and admin (`/admin`, adminMiddleware): `GET /` (app catalog + disabled list), `PUT /disabled` (set global kill-switch), `PUT /tiers` (set single app tier), `PUT /tiers/bulk` (set multiple app tiers).

**Admin UI:** Settings → **Apps** (admin) = global per-app kill switch (Settings always on). Settings → **Tiers & Plans** (admin) = assign each app to a tier (free/paid/pro) + configure Stripe price IDs. Settings → **Users** → edit user = role assignment (Free/Paid/Pro/Manager/Demo/Admin).

**Locked states:** Preview-mode apps render the real app content with `pointerEvents: none` and a `PaywallOverlay` on top. The overlay can be dismissed to browse the preview, but a compact lock badge remains.
...
## Echo (Pro-tier live lecture companion)

A dedicated **Echo** app (`client/src/apps/echo/EchoApp.tsx`) is the third Pro-tier-exclusive app. It's a real-time lecture companion: the user hits "Start lecture" and Echo streams their mic (or system audio) through ASR, transcribing live. As terms are spoken, Echo surfaces related concepts from the user's Atlas in real time — green chips = known concepts, red chips = weak concepts (mastery < 60%). At the end, it produces a structured note (reusing the existing `LectureJob` note-style pipeline) and suggests flashcards for new terms not in the user's Atlas.

**Distinct from:** `LectureJob` (offline, processes a recorded video file after the fact — Echo is the live, in-the-room version), Voice Notes (generic recording, no concept matching), Study Hub (per-document study, not live), Podcast (generates a podcast from a source, not a live companion).

**Data model:** `EchoSession` Prisma model — one row per lecture session (a user can have many over time but only one active at a time). JSON columns: `transcript` (array of `{ start, end, text }` segments, absolute seconds from session start), `concepts` (array of matched Atlas concepts with `{ id, label, type, mastery, weak, firstMentionedSec, mentionCount, definition }`), `newTerms` (array of `{ term, context, suggestedFront, suggestedBack }` — extracted by the LLM on stop). `noteId` is set when the session is stopped and the LLM generates a structured Note. Status: `active` | `completed` | `failed`.

**Recording loop (client):** `MediaRecorder` (audio/webm;codecs=opus) captures audio from `getUserMedia({ audio: true })`. Data is collected every 1s; every 8s the accumulated chunks are sent as a multipart POST to `/api/echo/sessions/:id/chunk` with the current `offsetSec` (seconds from session start). The client polls `GET /api/echo/sessions/:id` every 3s for the updated transcript + concepts. On stop, any remaining chunks are flushed, then `POST /api/echo/sessions/:id/stop` triggers the LLM finalization.

**Chunk processing (server, `services/echo.ts` `processChunk`):** (1) Transcribe the audio chunk via the OpenAI-compatible Whisper endpoint (reuses `getTranscriptionConfig` from `services/study/lecture/transcribe.ts` — same per-user / server-wide config hierarchy). (2) Append the new segment(s) to the transcript JSON. (3) Re-match concepts against the full transcript: for each concept label in the user's Atlas, check if it appears in the transcript (reusing Atlas's `textContains` word-boundary matching logic), compute `mentionCount` + `firstMentionedSec`. (4) Update the session row synchronously. If the user has no Atlas, concepts is always empty — Echo still works as a live transcriber + note generator.

**Stop + finalize (server, `services/echo.ts` `stopSession`):** (1) Run an LLM pass to generate a structured note from the full transcript (reuses `lectureSlideNotePrompt` in transcript-only mode — same fallback path as the lecture pipeline when no slides are detected). (2) Extract new terms: one `generateJson` call that takes the transcript + the user's existing Atlas concept labels and returns up to 10 terms NOT in their Atlas, each with a context snippet + suggested flashcard front/back. (3) Save the note (tagged `lecture,live,ai-generated`). (4) Flip status to `completed`. (5) Log a `StudySession` (`type: "lecture_notes"`).

**Privacy:** Audio chunks are processed ephemerally — sent to the Whisper endpoint for transcription and not stored on disk. Only the transcript text is persisted in the `EchoSession` row.

**Endpoints** (`routes/echo.ts`): `POST /sessions` (start — or reuse active), `GET /sessions/active`, `GET /sessions/:id`, `GET /sessions` (list completed), `POST /sessions/:id/chunk` (multipart audio upload), `POST /sessions/:id/stop` (finalize via LLM), `DELETE /sessions/:id`. All gated by `echoGate` middleware → 402 for non-Pro users.

**Athena tools** (`tools/echo.ts`, all `proOnly: true`): `echo_active_session` (check if a live session is active + transcript stats + matched concepts from Atlas), `echo_session_history` (list past captured lectures), `echo_session_detail` (get a past session's full transcript + concepts + new terms with suggested flashcards), `open_echo` (client action — opens the Echo app). The `open_echo` client action is dispatched in `AthenaApp.tsx` alongside `open_atlas` / `open_crunch`.

**Athena context:** `workspaceSummary` in `context.ts` includes an "Echo: live lecture active (Nm, N words, N weak concepts mentioned)" line when a session is active. The system prompt includes an Echo capabilities line (Pro-only, hidden for non-Pro users) that instructs Athena to proactively ask if the user wants a summary of what's been covered when a live session is active.

**UI:** Desktop `EchoApp` has two views — "Live" (recording controls + real-time scrolling transcript with timestamps + sidebar showing matched concepts as colored chips with mastery badges + new terms with suggested flashcards after stop) and "History" (list of past sessions with date/duration/word count/concept count, click to view). Mobile `MobileEcho` is a single-column flow: recording controls at top, transcript scrolling below, concepts as inline chips, new terms as expandable cards.
...
**UI:** Desktop `PulseApp` has per-exam readiness gauges (circular SVG, color-coded: green ≥80%, amber ≥60%, red <60%), a forecast curve chart (pure SVG — projected mastery over time with exam-date vertical markers and a 70% at-risk threshold line), an at-risk concept feed (sorted by days until forgotten, with "Review" → opens Flashcards app on the linked deck, and "Crunch" → opens Crunch and surfaces the at-risk concept there), and a "days until forgotten" list (top 10 concepts sorted soonest-forgotten-first). Stale banner when new reviews or Crunch changes happened since last build. Empty state explains the feature + "Build my forecast" CTA (notes no AI provider needed). The app is responsive (grid breakpoints for gauges) and works on mobile.

**Pulse → Crunch integration:** The at-risk feed's "Crunch" button opens Crunch and passes the at-risk concept (label + linked deck ids) via a `crunch:pulse-at-risk:<windowId>` sessionStorage key (same signal pattern as `crunch:focus:`). `CrunchApp` reads this on mount and renders a red banner: "Pulse predicts <concept> will drop below mastery before your exam" with a "Review now" button (opens Flashcards on the linked deck) and a dismiss (X). This closes the Pulse→Crunch loop described in the plan — the user lands in Crunch with the at-risk concept highlighted and an immediate review CTA.
